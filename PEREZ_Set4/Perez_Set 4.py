...
def bin_to_dec(binary_string):
    ...
    decimal_value = 0 
    string_length = len(str(binary_string))
    for i in range(string_length):
        bit = int(binary_string[i])
        power = string_length - 1 - i
...
def dec_to_bin(number):
    ...
    while number != 0:
        ...
        binary_string = str(number_modulo) + binary_string
        number = number//2
...
def telephone_cipher(message):
    ...
    for character in range(len(message)):
        current_character = message[character]
        if character > 0 and encoder_dict[current_character][0] == encoder_dict[message[character - 1]][0]:
                numerical_string += "_"
        numerical_string += encoder_dict[message[character]] 
    return str(numerical_string)
    ...
    while i < len(telephone_string):
        if telephone_string[i] == "_":
            ...
            while i + 1 < len(telephone_string) and telephone_string[i] == telephone_string[i + 1]:
                repeating_count += 1
                i += 1
            key = telephone_string[i]*repeating_count
            deciphered_message += decipher_dict[key]
            i+=1
